/**
 * Cloudflare Worker Integration for Google Sheets and Mailchimp
 * 
 * This worker should be deployed to your existing Cloudflare Worker
 * at: https://podcast-question-proxy.hugo-3ec.workers.dev
 * 
 * Add this route handler to your existing worker:
 */
  ...
  async fetch(request, env) {
    ...
    if (url.pathname === '/submit-email' && request.method === 'POST') {
      try {
        ...
        // Add to Google Sheets via Apps Script webhook
        ...
        const mailchimpResult = await addToMailchimp(email, name, env);
    ...
    // Handle existing OpenAI proxy route
    // ... (your existing code)
  }
};
...
async function addToMailchimp(email, name, env) {
  ...
  try {
    ...
    // Calculate MD5 hash of email (lowercase, trimmed)
    const emailHash = await md5Hash(email.toLowerCase().trim());
    
    // Create or update member using PUT (idempotent)
    const memberUrl = `https://${serverPrefix}.api.mailchimp.com/3.0/lists/${listId}/members/${emailHash}`;
    ...
    // Add tags to the member using the email hash
    if (tags.length > 0) {
      const tagsUrl = `https://${serverPrefix}.api.mailchimp.com/3.0/lists/${listId}/members/${emailHash}/tags`;
      ...
      if (!tagsResponse.ok) {
        ...
        console.warn('Failed to add tags, but member was added:', errorText);
 ...
 * Proper MD5 implementation for Cloudflare Workers
 */
async function md5Hash(str) {
  // MD5 implementation for Cloudflare Workers
  // This is a proper MD5 hash function
  return md5(str);
}
